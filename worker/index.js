...
async function handleUpscale(request, corsHeaders) {
    ...
    const upscaleScale = allowedScales.includes(requestedScale) ? requestedScale : 8;

    if (scale && upscaleScale !== requestedScale) {
...
async function handleBatchGenerate(request, corsHeaders) {
    ...
    try {
        ...
        const batchResponse = await fetch(
            `https://generativelanguage.googleapis.com/v1beta/models/gemini-3.1-flash-lite-image:batchGenerateContent`,
            {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
