...
export const generateFlyerImage = async (
  ...
  let prompt = `
    ...
    【★重要：ターゲット層★】
    このチラシのターゲットは60歳以上のシニア層です。以下を必ず守ってください：
    - 文字は大きめに（最低でも本文14pt相当以上、見出しは特に大きく）
    - 文章は短く、要点のみをわかりやすく
    - 専門用語は避け、シンプルな表現を使用
    - 視認性を最優先（コントラストを高く、読みやすい配色）
    - 情報を詰め込みすぎない（余白を十分に確保）
    ...
    【★テキスト品質要件★】
    高画質で、文字が一切崩れないように、日本語テキストを正確かつ鮮明に表示してください。
    ・解像感の高い、シャープでクリアな仕上がり
    ・日本語フォントは可読性重視（ゴシック体・均一な線）
    ・文字の欠け、潰れ、誤変換、崩れを絶対に起こさない
    ・図と文字の間に十分な余白を確保
    ・SNS投稿で一目で理解できる構成
    ・年配の方にも見やすいデザイン、フォントサイズ、構成
